...
# Apache.
#   usage: ./merge_parquet_pr.py    (see config env vars below)
#
# This utility assumes you already have a local Parquet git clone and that you
# have added remotes corresponding to both (i) the GitHub Apache Parquet mirror
...
# PARQUET_GITHUB_API_TOKEN: a GitHub API token to use for API requests (to avoid
# rate limiting)
...
# We can merge PARQUET patches
SUPPORTED_PROJECTS = ['PARQUET']
...
class GitHubAPI(object):
    ...
    def __init__(self, project_name):
        ...
        token = os.environ.get('PARQUET_GITHUB_API_TOKEN', None)
        if token:
...
CONFIG_FILE = "~/.config/parquet/merge.conf"
...
def cli():
    # Location of your Parquet git clone
    PARQUET_HOME = os.path.abspath(os.path.dirname(__file__))
    PROJECT_NAME = os.environ.get('PARQUET_PROJECT_NAME') or 'parquet-format'
