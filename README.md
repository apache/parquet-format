...
chunks they are interested in.  The columns chunks should then be read sequentially.
 ...
 over the various footer size vs compute trade-offs.  Its format is as follows:
   ...
   - 8-byte little-endian flag field to indicate features that require special parsing of the footer.
  ...
  When parsing the footer implementations SHOULD read at least the last 16 bytes of the footer. Then
  ...
  for PAR1 files. Any ambiguity can be completely eliminated if the CRC32 is written in PAR3 mode and verified by readers.
