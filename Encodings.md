...
frame of reference, then bit-packing the deltas at an optimal bit width selected
by a cost model. Values that do not fit in the chosen bit width are stored as
exceptions ("patches"). The frame is any value of the column's type, so it may
be the minimum of the vector or a value above it that leaves the outliers to be
patched on either side. The cost model trades off narrower bit-packing
