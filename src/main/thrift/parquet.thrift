...
/**
 * Parquet wire types. These types are intended to be used in combination with
 * the encodings to control the on disk storage format.
 *
...
enum FieldRepetitionType {
  /** This field is required (non-null) and each row has exactly 1 value */
  REQUIRED = 0;

  /** The field is optional (nullable) and each row has 0 or 1 values */
  OPTIONAL = 1;
...
struct StringType {}  // allowed for BYTE_ARRAY, must be UTF-8 encoded8
struct UUIDType {}    // allowed for FIXED[16], must be raw UUID encoded
struct MapType {}     // see LogicalTypes.md
struct ListType {}    // see LogicalTypes.md
struct EnumType {}    // allowed for BYTE_ARRAY, must be UTF-8 encoded
struct DateType {}    // allowed for INT32
struct Float16Type {} // allowed for FIXED[2], must be FLOAT16 encoded
...
struct SchemaElement {
   ...
   * the values (e.g. a low cardinality INT col could have this set to 3). Note
  ...
  /**
   * Nested fields.  Since thrift does not support nested fields, the nesting
   * is flattened to a single list by a depth-first traversal. The children
   * count is used to construct the nested relationship. This field is not set
   * when the element is a primitive type
   */
 ...
 * repetition levels.
 *
 * See Encodings.md for the details of the more complicated encodings.
 */
enum Encoding {
  /**
   * Default encoding.
   ...
   * BYTE_ARRAY - 4 byte length (little-endia), followed by bytes.
   * FIXED_LEN_BYTE_ARRAY - Just the bytes.
   */
  PLAIN = 0;

  /**
   * DEPRECATED: Group VarInt encoding for INT32/INT64.
   * This was never used.
   */
  //  GROUP_VAR_INT = 1;
   ...
   * in the plain type. Prefer RLE_DICTIONARY instead.
   */
  PLAIN_DICTIONARY = 2;
  ...
  /**
   * DEPRECATED: Bit packed encoding.  This can only be used if the data has a
   * known max width. Prefer RLE instead.
   */
  BIT_PACKED = 4;
  ...
  /** Dictionary encoding: the ids are encoded using RLE encoding. */
  RLE_DICTIONARY = 8;
   ...
   * afterwards.
   *
   * 2.8 added FLOAT and DOUBLE
   * 2.11 added INT32, INT64, FIXED_LEN_BYTE_ARRAY
   */
  BYTE_STREAM_SPLIT = 9;
}
 ...
 * page can be placed in a column chunk.
 */
...
struct DataPageHeaderV2 {
   ...
   *
   * Ergo, num_values_in_data_section = num_non_null = num_values - num_nulls
    */
  2: required i32 num_nulls
  /**
   * Number of rows in this data page. Every page must begin at a row boundary
   * (repetition_level = 0): rows must **not** be split across page boundaries
  ...
  /** Length of the definition levels (RLE encoded) */
  5: required i32 definition_levels_byte_length;
  /** Length of the repetition levels (RLE encoded) */
  6: required i32 repetition_levels_byte_length;

  /**
   * Whether the values are compressed. Only payload is compressed, def/rep
   * levels are not.
   */
...
struct ColumnChunk {
   ...
   * location pointed to by file_path/file_offset. Note: while marked as
...
struct RowGroup {
  ...
  /**
   * If set, specifies a sort ordering of the rows in this RowGroup. The
   * sorting columns can be a subset of all the columns.
   */
...
struct AesGcmV1 {
  ...
  /**
   * In files encrypted with AAD prefix without storing it, readers must supply
   * the prefix.
   */
...
struct FileMetaData {
   ...
   * <Application> version <App Version> (build <App Build Hash>).* e.g. impala
   * version 1.0 (build 6cf94d29b2b7115df4de2c06e2ab4326d721eb55)
   */
  ...
  /** 
   * Encryption algorithm. This field is set only in encrypted files with
   * plaintext footer. Files with encrypted footer store algorithm id in
   * FileCryptoMetaData structure.
   */
  8: optional EncryptionAlgorithm encryption_algorithm

  /** 
   * Retrieval metadata of key used for signing the footer. Used only in
   * encrypted files with plaintext footer. 
   */ 
...
struct FileCryptoMetaData {
  /** 
   * Encryption algorithm. This field is only used for files with encrypted
   * footer. Files with plaintext footer store algorithm id inside footer
...
enum ConvertedType {
  ...
  /**
   * a list is converted into an optional field containing a repeated field for
   * its values
   */
  LIST = 3;
